...
WAGO_VL_SOLL_REG = 12384  # Soll-Vorlauftemperatur × 100 (OSCAT, Modbus TCP)
...
def fetch_wago_data(log) -> dict:
    """Liest WAGO Modbus TCP: Außentemperatur (Reg 12347) + OSCAT VL-Soll (Reg 12384).
    Gibt dict mit 'outdoor_temp_c' und 'wago_vl_soll_c' zurück (None bei Fehler)."""
    result = {"outdoor_temp_c": None, "wago_vl_soll_c": None}
    try:
        c = ModbusTcpClient(WAGO_HOST, port=WAGO_MODBUS_PORT, timeout=2)
        if not c.connect():
            log.warning("WAGO Modbus TCP: Verbindung fehlgeschlagen")
            return result
        for reg, key in ((WAGO_TEMP_REG, "outdoor_temp_c"),
                         (WAGO_VL_SOLL_REG, "wago_vl_soll_c")):
            rr = c.read_holding_registers(reg, count=1, slave=1)
            if rr.isError():
                log.warning(f"WAGO Reg {reg}: Lesefehler {rr}")
                continue
            raw = rr.registers[0]
            if raw >= 32768:
                raw -= 65536      # signed INT16
            result[key] = round(raw / 100.0, 1)
        c.close()
    except Exception as e:
        log.warning(f"WAGO Lesefehler: {e}")
    return result


# ─── Heizkurve (OSCAT HEAT_TEMP) + Ölbrenner-Pseudo-Relais ──────────────────
BIVALENZ_SETPOINT_C  = 39.0   # WP-Maximaltemperatur; darüber schaltet Ölbrenner zu
OELBRENNER_TARGET_C  = 66.0   # Ölbrenner heizt Puffer auf diesen Wert
OELBRENNER_HYS_ON_K  =  1.0   # Ölbrenner EIN  wenn vl_soll > BIVALENZ + 1 K
OELBRENNER_HYS_OFF_K =  1.0   # Ölbrenner AUS  wenn vl_soll < BIVALENZ - 1 K
...
def heat_curve(t_ext: float,
               ...
               c:        float =  1.33,
               ty_min:   float = 25.0,
               ty_max:   float = 70.0) -> float:
    """Heizkurve nach OSCAT HEAT_TEMP (exakte Formel mit Kurvatur-Exponent C).
    TY = TR + T_DIFF_C/2·TX + (TY_C - T_DIFF_C/2 - TR)·TX^(1/C)
    TR = T_INT + OFFSET,  TX = (TR - T_EXT) / (T_INT_C - T_EXT_C)
    """
    tr = t_int + offset
...
def oelbrenner_check(outdoor_temp_c: float | None, state: bool | None, log) -> bool | None:
    """Pseudo-Relais Ölbrenner: EIN/AUS basierend auf Heizkurve. Nur bei Zustandswechsel."""
    if outdoor_temp_c is None:
        return state
    vl_soll = heat_curve(outdoor_temp_c)
    if state is not True and vl_soll > BIVALENZ_SETPOINT_C + OELBRENNER_HYS_ON_K:
        log.info(f"Oelbrenner ON  → Puffer {OELBRENNER_TARGET_C}°C"
                 f"  [vl_soll={vl_soll}°C > {BIVALENZ_SETPOINT_C + OELBRENNER_HYS_ON_K}°C"
                 f"  t_out={outdoor_temp_c}°C]")
        return True
    if state is not False and vl_soll < BIVALENZ_SETPOINT_C - OELBRENNER_HYS_OFF_K:
        log.info(f"Oelbrenner OFF [vl_soll={vl_soll}°C < {BIVALENZ_SETPOINT_C - OELBRENNER_HYS_OFF_K}°C"
                 f"  t_out={outdoor_temp_c}°C]")
        return False
    return state


def mqtt_publish(results: dict, shelly_state, volumeflow, power_gwp, power_hp, power_zenner, cop_avg_2h, outdoor_temp_c, wago_vl_soll_c, log):
    ...
    payload["wago_vl_soll_c"]  = wago_vl_soll_c
    for reg, info in ERROR_REGS.items():
        if reg in results:
...
def db_insert(results: dict, cop_2h, outdoor_temp_c, wago_vl_soll_c, log):
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            if outdoor_temp_c is not None:
                ...
                ph.append("%s")
            if wago_vl_soll_c is not None:
                cols.append("wago_vl_soll_c")
                vals.append(wago_vl_soll_c)
                ph.append("%s")
            cur.execute(
...
def main():
    ...
    oelbrenner_state  = None   # None = unbekannt, True = EIN, False = AUS (Pseudo-Relais)
    last_db_time      = 0.0
    last_db_insert    = 0.0
    last_freq_time    = 0.0
    ...
    while True:
        try:
            ...
            if now - last_db_time >= DB_SEC:
                ...
                wago           = fetch_wago_data(log)
                outdoor_temp_c = wago["outdoor_temp_c"]
                wago_vl_soll_c = wago["wago_vl_soll_c"]
                oelbrenner_state = oelbrenner_check(outdoor_temp_c, oelbrenner_state, log)
                # 60s DB-Insert mit aktuellem COP
                if now - last_db_insert >= 60:
                    last_db_insert = now
                    _cached_cop_2h = fetch_avg_cop_2h(log)
                    db_insert(results, _cached_cop_2h, outdoor_temp_c, wago_vl_soll_c, log)
                mqtt_publish(results, shelly_state, volumeflow, power_gwp, power_hp, power_zenner, _cached_cop_2h, outdoor_temp_c, wago_vl_soll_c, log)
